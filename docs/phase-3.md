...
历史压缩在第九个保留真人轮触发，一批压缩较早的若干完整轮，保留最近五轮及新输入；这满足约八轮上限且避免“压缩一条短回答后摘要反而更大”。摘要最大64 tokens由发行preset配置。原文仍在DSH日志；摘要绝不进入抽取来源。

若原生压缩明确返回摘要不比原文短，仅当完整保留surface内容≤8000字符时允许继续短对话，至少新增8个surface事件再尝试；这是约八轮目标的有界短文本例外。超预算或其它压缩故障显式失败，不无界保留历史。无缩减拒绝本身由DSH原生日志记录。
