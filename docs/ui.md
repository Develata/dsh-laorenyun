...
河流刷新以Host `projectionRevision`为准，覆盖图、关系、memo、来源数和冲突展示。日期400/漂流100独立预算与分页；跨年代选择先换查询、再等待新几何定位。右预览仅挂载时每5秒读有界投影，取消/隐藏行为见`river-refresh.ts`。无关系漂流节点不连树，最多12组画布，其余列表。

RC5版式保持；线SVG替代单字rail，完整标签/原生tooltip保留。详情按时间/标题、关联、原始讲述、次要更正排列；有自传时表达方式折叠。Web和阅读导出共用纯展示标题，历史书稿不修改。
