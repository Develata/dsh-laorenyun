...
export interface FactAtom {
  ...
  /** Active support only; the fixed graph manifest retains complete history. */
  sourceRefs: string[];
  historyRefs?: string[];
...
export function factManifest(m: Manifest): FactAtom[] {
  return m.nodes
    ...
    .map((n, i) => {
      ...
      // Same-node corrections carry inherited evidence by design. Use the latest
      // explicit revision boundary, never text/year heuristics, to retire it from
      // active narrative support. Separate-node resolutions keep the selected
      // side's evidence. Missing history fails closed rather than citing it all.
      const correction = conflicts
        .filter(
          (c) =>
            c.status === "resolved" &&
            c.resolution?.selectedNodeId === n.id &&
        ...
        .sort((a, b) => b.right.revision - a.right.revision)[0];
      const prior = correction
        ? m.revisions.find(
            (r) =>
              r.id === correction.left.id &&
              r.revision === correction.left.revision,
          )
        : undefined;
      if (correction && !prior) fail("correction history unavailable");
      const inherited = new Set(
        prior?.evidence.map((e) => e.transcriptId) ?? [],
      );
      const activeEvidence = evidence.filter(
        (e) => !inherited.has(e.transcriptId),
      );
      const activeClarifications = clarifications.filter(
        (c) => !inherited.has(c.transcriptId),
      );
      const sourceRefs = [
        ...new Set([
          ...activeEvidence.map((e) => e.transcriptId),
          ...activeClarifications.map((c) => c.transcriptId),
        ]),
      ].sort();
      if (!sourceRefs.length) fail("current narrative support unavailable");
      const historyRefs = [
        ...new Set([
      ...
      return {
        ...
        time,
        sourceRefs,
        ...(historyRefs.length > sourceRefs.length ? { historyRefs } : {}),
        testimony,
        sourceMode,
        narrativePolicy:
        ...
        attribution: {
          ...
          sourceLabels: roles
            ...
            .map(
              (r) =>
                ({
                  child: "传主的子女；不是传主小时候，也不表示证言发生于童年",
                  spouse: "传主的配偶；不是传主本人",
                  friend: "亲友提供的证言；不是传主本人",
                  other: "其他讲述者提供的证言；不是传主本人",
                })[r] ?? "其他讲述者",
            ),
        },
        temporalPolicy: {
...
function matchesSourceRole(surface: string, roles: string[]): boolean {
  if (/小时候|孩子时|童年|儿时|的我|我自己/u.test(surface)) return false;
  return roles.some((role) => {
    if (
      ["child", "spouse", "friend"].includes(role) &&
      /家人|家里人|家中人/u.test(surface)
    )
      return true;
    const pattern = {
      child: /孩子|子女|儿子|女儿/u,
      spouse: /配偶|爱人|伴侣|丈夫|妻子/u,
      friend: /亲友|朋友|亲戚/u,
      other: /他人|别人|其他.*讲述者/u,
    }[role];
...
export function paragraphProblems(
  ...
  for (const f of facts.filter((f) => f.attribution.required)) {
    ...
    if (
      ...
      !matchesSourceRole(a.surface, f.attribution.speakerRoles)
    )
